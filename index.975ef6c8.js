...
const renderPosts = (posts)=>{
    postsList.innerHTML = posts.map((post)=>`
        <li class="posts__item">
            ...
            <button class="edit__post" data-id="${post.id}">\u{270F}\u{FE0F}</button>
            <button class="delete__posts" data-id="${post.id}">\u{274C}</button>
        </li>
    `).join("");
};
...
const updatePost = async ()=>{
    try {
        ...
        await (0, _axiosDefault.default).put(`${API_URL}/${id}`, {
            name,
            text
        });
        editNameInput.value = "";
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["farZc","8lqZg"], "8lqZg", "parcelRequirefbf3")
